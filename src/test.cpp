...
init() {
  ...
  g = Graph(1, 1);
  if ((g.getVertices() == 1) && (g.legal_edge(0, 0)) && g.setEdge(0, 0) &&
      (g.getEdges() == 1)) {
    std::cout << "OK" << std::endl;
  } else {
  ...
  g = Graph(3, 3);
  if ((g.getVertices() == 3) && g.setEdge(0, 1) && g.setEdge(0, 2) &&
      g.setEdge(1, 2) && g.neighbours(0, 1) && g.neighbours(2, 0) &&
      g.neighbours(1, 2) && (g.getEdges() == 3)) {
    std::cout << "OK" << std::endl;
  } else {
    g.print();
